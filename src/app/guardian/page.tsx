...
const LiveMapTracker = dynamic(
  () => import('@/components/guardian/LiveMapTracker'),
  {
    ssr: false,
    loading: () => (
      <div className="w-full h-80 rounded-2xl bg-amber-50/80 animate-pulse flex flex-col items-center justify-center border-2 border-dashed border-amber-300 gap-2">
        <Map className="w-10 h-10 text-amber-500 animate-bounce" />
        <span className="text-amber-900 font-bold text-sm sm:text-base">
          सुरक्षित क्षेत्र नक्शा लोड हो रहा है... (Loading OpenStreetMap Engine...)
        </span>
      </div>
    ),
  }
